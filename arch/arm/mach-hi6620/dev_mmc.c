...
static struct hisik3_mmc_platform_data sd_plat_data = {
	...
	.reg_name_lvs = NULL,/*ldo7*/
	.signal_reg_name = NULL,/*ldo22*/
...
int bsp_config_write(unsigned int config_id_addr,unsigned int size,unsigned char* buffer)
{
    void __iomem * config_kernel_addr = NULL;

    if(NULL == buffer)
    {
        pr_err("bsp_config_write:buffer is NULL!\n");
        return -1;
    }

    if(0 == size)
    {
        pr_err("bsp_config_write:size is NULL!\n");
        return -1;
    }

    if((config_id_addr+size) > (MEMORY_AXI_CONFIG_ADDR + MEMORY_AXI_CONFIG_SIZE))
    {
        pr_err("bsp_config_write:config_id is over!\n");
        return -1;
    }

    config_kernel_addr = ioremap(config_id_addr,size);
    if(NULL == config_kernel_addr)
    {
        pr_err("bsp_config_write ioremap \n");
        return -1;
    }

    memcpy((char*)config_kernel_addr,buffer,size);
...
static int __init plat_mmc_dev_init(void)
{
	...
	if((support_sdcard) && (support_cbp)){
	   ...
	   ret = platform_add_devices(plat_mmc_dev3, ARRAY_SIZE(plat_mmc_dev3));
    	   ...
    	   (void)bsp_config_write(CONFIG_IS_SDCARD_EXIST_ADDR,CONFIG_IS_SDCARD_EXIST_SIZE,(unsigned char*)&support_sdcard);
	   pr_info("%s has sd card support, has no cbp support\n", __func__);
	   ret = platform_add_devices(plat_mmc_dev, ARRAY_SIZE(plat_mmc_dev));
	}else{
       /* platform without sd need to pd ldo7 and ldo22 first which is on by default */
       pmussi_reg_write(SOC_SMART_DISABLE2_ADDR(0),BIT(SOC_SMART_ENABLE2_en_ldo7_int_START));
       pmussi_reg_write(SOC_SMART_DISABLE4_ADDR(0),BIT(SOC_SMART_ENABLE4_en_ldo22_int_START));
