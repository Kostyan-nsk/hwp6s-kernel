...
#endif
    PWC_TRACE(PWC_LOG_ERROR, "can deep sleep ret: 0x%x, IPC wake status: 0x%x,\r\n", \
         g_stMcuLogExt->mcuCanDeepSleepRet, g_stMcuLogExt->peri.wakesrcIPC);

    PWC_TRACE(PWC_LOG_ERROR, "GPIO wake status: (GPIO7-4)0x%08x, (GPIO3-0)0x%08x,\r\n", \
         *((u32_t*)&(g_stMcuLogExt->wakesrcGPIO[4])), *((u32_t*)&(g_stMcuLogExt->wakesrcGPIO[0])));
