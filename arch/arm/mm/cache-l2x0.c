...
unsigned int l2cc_get_intr(void)
{	
	void __iomem *base = l2x0_base;

	return readl_relaxed(base + L2X0_RAW_INTR_STAT);
}
unsigned int __iomem l2cc_reg_save[10];
static inline void cache_sync(void)
{
	void __iomem *base = l2x0_base;
    l2cc_reg_save[0]=(readl(base + L2X0_CACHE_SYNC) & 1);
    l2cc_reg_save[1]=(readl(base + L2X0_INV_LINE_PA) & 1);
    l2cc_reg_save[2]= readl(base + L2X0_INV_WAY);
    l2cc_reg_save[3]=(readl(base + L2X0_CLEAN_LINE_PA) & 1);    
    l2cc_reg_save[4]=(readl(base + L2X0_CLEAN_LINE_IDX) & 1);
    l2cc_reg_save[5]=readl(base + L2X0_CLEAN_WAY);
    l2cc_reg_save[6]=(readl(base + L2X0_CLEAN_INV_LINE_PA) & 1);
    l2cc_reg_save[7]=(readl(base + L2X0_CLEAN_INV_LINE_IDX) & 1);
    l2cc_reg_save[8]=readl(base + L2X0_CLEAN_INV_WAY);

	if( l2cc_reg_save[0]||l2cc_reg_save[1]||l2cc_reg_save[2]||l2cc_reg_save[3]||l2cc_reg_save[4]||l2cc_reg_save[5] \
	||l2cc_reg_save[6]||l2cc_reg_save[7]||l2cc_reg_save[8])
	{
        printk(KERN_ERR "l2cc_reg_save[0]:0x%x,[1]:0x%x,[2]:0x%x,[3]:0x%x,[4]:0x%x,[5]:0x%x,[6]:0x%x,[7]:0x%x,[8]:0x%x\n",
            l2cc_reg_save[0],l2cc_reg_save[1],l2cc_reg_save[2],l2cc_reg_save[3],l2cc_reg_save[4],l2cc_reg_save[5],l2cc_reg_save[6],l2cc_reg_save[7],l2cc_reg_save[8]);
	    BUG_ON(1);
	}
...
void __init l2x0_init(void __iomem *base, __u32 aux_val, __u32 aux_mask)
{
    ...
    /*enable l2cache int*/
	aux = readl_relaxed(l2x0_base + L2X0_INTR_MASK);
	aux |= (3<<7);
    writel_relaxed(1, l2x0_base + L2X0_INTR_MASK);	
