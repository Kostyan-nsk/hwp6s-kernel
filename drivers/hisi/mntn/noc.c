...
static irqreturn_t noc_irq(int irqno, void *param)
{
    ...
    void __iomem *l2cc_base_map_addr = (void *)IO_ADDRESS(REG_BASE_L2CC);
    ...
    /*l2cc int*/
    if(reg&BIT(SOC_AO_SCTRL_SC_SECONDRY_INT_STATM0_sc_secondry_int_mask0_18bit_START))
    {
        /*print l2cc int stat reg*/
        printk(KERN_ERR"err in l2cc,L2X0_MASKED_INTR_STAT:0x%x,L2X0_RAW_INTR_STAT:0x%x.\n",readl(l2cc_base_map_addr+0x218),readl(l2cc_base_map_addr+0x21c));
        /*set L2X0_INTR_CLEAR,clr int*/
        val = BIT(7) | BIT(8);
        writel(val,l2cc_base_map_addr+0x220);       
...
static int __init noc_init(void)
{
    ...
    reg |= BIT(SOC_AO_SCTRL_SC_SECONDRY_INT_EN0_sc_secondry_int_en0_18bit_START);
