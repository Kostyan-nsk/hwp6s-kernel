        ...
        unsigned int size)
{
    ...
    if (!list_empty(free_pool)) {
        ...
        rc = v2p_get_pa((unsigned long)buf, (unsigned long *)&dma_addr);
        if (NCPY_OK == rc) {
...
#ifndef _DRV_LLT_
/* 实现虚拟地址到物理地址转换 */
int v2p_get_pa(unsigned long va, unsigned long *pa)
{
    pgd_t *pgd;
   	pud_t *pud;
   	pmd_t *pmd;
...
int v2p_test_dma_alloc(int size)
{
    void *vir = NULL;
    dma_addr_t phy = 0;
    unsigned long  phy_get = 0;

    vir = dma_alloc_coherent(0, size, &phy, GFP_KERNEL);
    v2p_get_pa((unsigned long)vir, &phy_get);
    printk("dma alloc size[0x%x] vir[0x%x] phy[0x%x] phy_get[0x%lx]\n",
        size, (unsigned long)vir, phy, phy_get);
    return NCPY_OK;
}
...
int v2p_test_kmalloc(int size)
{
    ...
    printk("dma alloc size[0x%x] vir[0x%lx] phy[0x%x] phy_get[0x%lx]\n",
        size, (unsigned long)vir, phy, phy_get);
    return NCPY_OK;
}

void v2p_test_get_pa(unsigned long va)
{
    unsigned long pa;
