...
#define NFC_CONF_NAME_LEN   32
#define MAX_BUFFER_SIZE	512
#define NXP_DEFAULT_CONF			"libnfc-nxp.conf"
#define NXP_DEFAULT_CONF_TAIL	"-libnfc-nxp.conf"

static int firmware_update = 0;
char nfc_firmware_version[NFC_FWVERSION_LEN] = {0};
char nfc_conf_name[NFC_CONF_NAME_LEN] = {0};
...
static ssize_t nfc_conf_show(struct device *dev, struct device_attribute *attr, char *buf)
{
        if (NULL != buf) {
                return (ssize_t)(snprintf(buf, strlen(nfc_conf_name)+1, "%s", nfc_conf_name));
        }
}
...
static struct device_attribute pn544_attr[] ={
	...
	__ATTR(nfc_conf, 0664, nfc_conf_show, NULL),
};
...
static int check_pn544(struct i2c_client *client)
{
       int ret = -1;
       int count = 0;
       const char host_to_pn544[1] = {0x20};
...
static void nfc_config_name_set(char *name)
{
	memset(nfc_conf_name, 0x00, sizeof(nfc_conf_name));
	if (strcmp(name, "default") == 0 || name == NULL) {
		strncpy(nfc_conf_name, NXP_DEFAULT_CONF, strlen(NXP_DEFAULT_CONF));
	} else {
		strcat(nfc_conf_name, name);
		strcat(nfc_conf_name, NXP_DEFAULT_CONF_TAIL);
	}
	printk("nxp config name:%s\n", nfc_conf_name);
}
		...
		const struct i2c_device_id *id)
{
	...
	nfc_config_name_set(platform_data->conf_name);
