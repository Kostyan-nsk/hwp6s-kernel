...
static s32 wl_prune_event_handler(struct wl_priv *wl, struct net_device *ndev,
	const wl_event_msg_t *e, void *data);
	...
	if (err == -EBUSY) {
		if (busy_count++ > SCAN_EBUSY_RETRY_LIMIT) {
			...
			if ((ret = wldev_ioctl(ndev, WLC_GET_BSSID,
				...
				WL_ERR(("WIFI_DBG, send hang message.\n"));
				net_os_send_hang_message(ndev);
			}
...
static s32 
wl_prune_event_handler(struct wl_priv *wl, struct net_device *ndev,
	const wl_event_msg_t *e, void *data)
{
	u32 reason = ntoh32(e->reason);
	dhd_pub_t *dhd;
...
static s32 
wl_clear_wrong_action_handle(struct wl_priv *wl, struct net_device *ndev, wl_extjoin_params_t *ext_join_params)
{
	dhd_pub_t *dhd;
