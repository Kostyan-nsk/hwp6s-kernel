...
static int __init lsm330_acc_i2c_init(void)
{
	pr_info("%s accelerometer driver: init\n", LSM330_ACC_DEV_NAME);
	return i2c_add_driver(&lsm330_acc_i2c_driver);
}

static void __exit lsm330_acc_i2c_exit(void)
{
	i2c_del_driver(&lsm330_acc_i2c_driver);
}

module_init(lsm330_acc_i2c_init);
module_exit(lsm330_acc_i2c_exit);
