...
static int lsm330_gyr_enable(struct lsm330_gyr_status *stat)
{
	...
	if (!atomic_cmpxchg(&stat->enabled, 0, 1)) {
		...
		if (err < 0) {
			...
			dev_info(&stat->client->dev, "lsm330_gyr_device_power_on error, return %d\n", err);
			return err;
		}
...
static int lsm330_get_average_data(struct lsm330_gyr_triple *data_ave)
{
	int i, ret;
	struct lsm330_gyr_triple data_out;
	int32_t data_sum[3] = {0,0,0};

	for (i = 0; i < 5; i++) {
		ret = lsm330_gyr_get_data(stat_temp, &data_out);
		if (ret < 0) {
			pr_err("%s lsm330_gyr_get_data failed\n", __func__);
			return ret;
		}
		pr_info("lsm330_get_average_data[index %d] %d %d %d\n", i, data_out.x, data_out.y, data_out.z);
		data_sum[0] += data_out.x;
		data_sum[1] += data_out.y;
		data_sum[2] += data_out.z;
		msleep(10);
	}
	data_ave->x = abs(data_sum[0]/5);
	data_ave->y = abs(data_sum[1]/5);
	data_ave->z = abs(data_sum[2]/5);

	return 0;
}

static int lsm330_check_selftest(void)
{
	int buf[2];
	int i;
	int data;
	struct lsm330_gyr_triple data_ave;
	struct lsm330_gyr_triple data_ave_selftest;
	int err;

	buf[0] = CTRL_REG4;
	buf[1] = 0x00;
	lsm330_gyr_i2c_write(stat_temp, buf, 1);
	msleep(100);

	err = lsm330_get_average_data(&data_ave);
	if (err < 0) {
		atomic_set(&selftest_err_code, 0x02);
		return SELF_TEST_FAIL;
	}
	dev_info(&stat_temp->client->dev, "data_ave: %d, %d, %d\n", 
		data_ave.x, data_ave.y, data_ave.z);
	...
	err = lsm330_get_average_data(&data_ave_selftest);
	if (err < 0) {
		atomic_set(&selftest_err_code, 0x03);
		return SELF_TEST_FAIL;
	}
	dev_info(&stat_temp->client->dev, "data_ave_selftest: %d, %d, %d\n", 
		data_ave_selftest.x, data_ave_selftest.y, data_ave_selftest.z);

	/*self_test value*/
	data = abs(data_ave_selftest.x - data_ave.x);
	if ((data > MAX_VAL) || (data < MIN_VAL)) {
		atomic_set(&selftest_err_code, 0x04);
		dev_err(&stat_temp->client->dev, "selftest_gyro MAX-MIN--X-= %d, %d, %d\n", data, data_ave_selftest.x, data_ave.x);
		return SELF_TEST_FAIL;
	}
	data = abs(data_ave_selftest.y - data_ave.y);
	if ((data > MAX_VAL) || (data < MIN_VAL)) {
		atomic_set(&selftest_err_code, 0x05);
		dev_err(&stat_temp->client->dev, "selftest_gyro MAX-MIN--Y =%d, %d, %d\n", data, data_ave_selftest.y, data_ave.y);
		return SELF_TEST_FAIL;
	}
	data = abs(data_ave_selftest.z - data_ave.z);
	if ((data > MAX_VAL) || (data < MIN_VAL)) {
		atomic_set(&selftest_err_code, 0x06);
		dev_err(&stat_temp->client->dev, "selftest_gyro MAX-MIN--Z =%d, %d, %d\n", data, data_ave_selftest.z, data_ave.z);
		return SELF_TEST_FAIL;
	}
...
int set_selftest_lm330(int value)
{
	...
	if (SELF_TEST == value) {
		...
		if (err) {
			atomic_set(&selftest_err_code, 0x01);
		...
		for (i = 0; i < 3; i++) {
			result = lsm330_check_selftest();
			if (result == SELF_TEST_OK) {
				break;
			} else {
				dev_err(&stat_temp->client->dev, "lsm330_check_selftest failed, try %d\n", i);
			}
		}
...
static int lsm330_gyr_later_resume(struct early_suspend *h)
{
	...
	dev_dbg(&client->dev, "%s\n", __func__);
	dev_info(&stat->client->dev, "%s  enter, on_before_suspend %d atomic_read(&stat->enabled) %d\n",
		__func__, stat->on_before_suspend, atomic_read(&stat->enabled));
