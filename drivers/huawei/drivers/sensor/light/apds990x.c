...
//       GA=5000;
	...
	else if (E_APDS_TYPE_H30L == apds_type)
	{  
		GA=5290;
		if(cdata < 214*irdata/100)
			GA = GA*125/110;
		else if(cdata >= 214*irdata/100 && cdata <= 50*irdata/10 )
			GA= GA *236/190;
		else
			GA = GA ;
...
static void apds990x_work_handler(struct work_struct *work)
{
	...
	i2c_smbus_write_byte_data(client, CMD_BYTE | APDS990x_ENABLE_REG, 1);
     printk("%s, status = %x ,irdata = %d,data->enable = %#x\n", __func__,status,irdata,data->enable);
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == 1) {
		...
		if (data->enable_ps_sensor == 0) {
			...
			if(E_APDS_TYPE_H30L==apds_type)
			{
				apds990x_set_pers(client, 0x23); /* 3 persistence */
			}
			else
			{
	 ...
	 else
	 {
		...
		if(E_APDS_TYPE_H30L==apds_type)
		{
			__cancel_delayed_work(&data->power_work);
		}
		if (data->enable_als_sensor) {
			...
			if(E_APDS_TYPE_H30L==apds_type)
			{
					apds990x_set_control(client, 0x60); /* 100mA, IR-diode, 1X PGAIN, 16X AGAIN */
					apds990x_set_pers(client, 0x23); /* 3 persistence */
			}
			else
			{
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == 1) {
		...
		if (data->enable_ps_sensor) {
			...
			if(E_APDS_TYPE_H30L==apds_type)
			{
				apds990x_set_control(client, 0x60);	
			}
			else
			{
            	apds990x_set_control(client, 0x20); /* 100mA, IR-diode, 1X PGAIN, 16X AGAIN */
...
static int apds990x_init_client(struct i2c_client *client)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (err < 0) {
			dev_err(&client->dev, "%s, apds990x_set_enable failed, retry %d\n",
					__func__, i);
			msleep(10);
		} else {
			dev_info(&client->dev, "%s, apds990x_set_enable success, retry %d\n",
					__func__, i);
			break;
		}
	}
				   ...
				   const struct i2c_device_id *id)
{
	...
	if(E_APDS_TYPE_H30L==apds_type)
	{
   		apds_990x_pwave_value = 108;
		threshold_value = 140;
		apds_990x_pwindows_value = 147;
 		ps_pulse_count=3;
	}	
	min_proximity_value = MAX_FAR_THRESHOLD;//MAX_ADC_PROX_VALUE - apds_990x_pwindows_value;
