...
struct touch_settings cypress_sett_param_regs_map[] = {
	[0]={
			.data = (uint8_t *)&cyttsp4_param_regs_h30l[0],
			.size = ARRAY_SIZE(cyttsp4_param_regs_h30l),
			.tag = 0,
...
struct touch_settings *cypress_sett_param_regs_h30l = &cypress_sett_param_regs_map[2];
...
static int cypress_chip_get_info(struct ts_chip_info_param *info)
{
	...
	{
		memcpy(info->mod_vendor, cypress_sett_param_regs_h30l->module_name, strlen(cypress_sett_param_regs_h30l->module_name));
	}
...
static int cypress_sysinfo_pcfg(void)
{
	...
	if (strcmp(g_ts_data.product_name,H30L_NAME) == 0) 
	{
		...
		for (count =0;cypress_sett_param_regs_map[count].module_name  !=NULL;count++)
		{
			...
			{
				cypress_sett_param_regs_h30l = &cypress_sett_param_regs_map[count];
				break;
			}
		}
		TS_LOG_INFO("module id is %s\n", cypress_sett_param_regs_h30l->module_name);
	}
...
static int cypress_get_crc_from_params(u16 *crc)
{
	...
	if (strcmp(g_ts_data.product_name,H30L_NAME) == 0) 
	{
		param_regs = cypress_sett_param_regs_h30l;
	}
	else
	{
		param_regs = &cypress_sett_param_regs;
	}	
	u16 crc_loc;
...
static bool cypress_need_fw_update(char *file_name)
{
	...
	if (strcmp(g_ts_data.product_name,H30L_NAME) == 0) 
	{
		rc = request_firmware(&fw_entry, cypress_sett_param_regs_h30l->file_name, dev);
	}
	else
	{
