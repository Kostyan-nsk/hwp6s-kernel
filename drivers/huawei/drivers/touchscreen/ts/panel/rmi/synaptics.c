...
#define CFG_GLOVEMODES_ON          0
#define CFG_GLOVEMODES_OFF         2
...
#define SYNAPTCS_X_RES "synaptics/x_res"
#define SYNAPTCS_Y_RES "synaptics/y_res"
...
static int synaptics_parse_dts(struct device_node *device,struct ts_device_data *chip_data)
{
	...
	rc = get_hw_config_int(SYNAPTCS_X_RES, &chip_data->max_x, NULL);
	if (!rc) {
		TS_LOG_ERR("read sensor_max_x config failed\n");
		goto err;
	}
	rc = get_hw_config_int(SYNAPTCS_Y_RES, &chip_data->max_y, NULL);
	if (!rc) {
		TS_LOG_ERR("read sensor_max_y config failed\n");
		goto err;
	}

	TS_LOG_DEBUG("reset_gpio = %d, irq_gpio = %d, irq_config = %d, max_x = %d, max_y = %d\n",
		chip_data->reset_gpio, chip_data->irq_gpio, chip_data->irq_config, chip_data->max_x, chip_data->max_y);
		...
		struct ts_device_data *chip_data, struct platform_device *ts_dev)
{
	...
	rmi4_data->sensor_max_x = rmi4_data->synaptics_chip_data->max_x - 1;
	rmi4_data->sensor_max_y = rmi4_data->synaptics_chip_data->max_y - 1;
	...
	TS_LOG_INFO("rmi4_data->sensor_max_x = %d,  rmi4_data->sensor_max_y =%d\n", rmi4_data->sensor_max_x, rmi4_data->sensor_max_y);
...
static int synaptics_get_glove_switch(u8 *glove_switch)
{
	...
	unsigned char ctrl_value = CFG_GLOVEMODES_OFF;
	...
	if (!strncmp(rmi4_data->rmi4_mod_info.product_id_string, "TM2451", strlen("TM2451"))) {
		glove_enable_addr = rmi4_data->rmi4_feature.f51_ctrl_base_addr;
		TS_LOG_DEBUG("get glove addr : 0x%2x\n",glove_enable_addr);

		retval = synaptics_rmi4_i2c_read(rmi4_data, glove_enable_addr, &ctrl_value,
			sizeof(ctrl_value));
		if (retval < 0) {
			TS_LOG_ERR("read glove ctrl_value(%d) err : %d\n", ctrl_value, retval);
			goto out;
		}

		if (ctrl_value == CFG_GLOVEMODES_ON)
			*glove_switch = 1;
		if (ctrl_value == CFG_GLOVEMODES_OFF)
			*glove_switch = 0;
		TS_LOG_INFO("read glove switch(%d)\n", *glove_switch);
	} else {
...
static int synaptics_set_glove_switch(u8 glove_switch)
{
	...
	if (!strncmp(rmi4_data->rmi4_mod_info.product_id_string, "TM2451", strlen("TM2451"))) {
		if (glove_switch == 1)
			ctrl_value = CFG_GLOVEMODES_ON;
		if (glove_switch == 0)
			ctrl_value = CFG_GLOVEMODES_OFF;

		glove_enable_addr = rmi4_data->rmi4_feature.f51_ctrl_base_addr;
		TS_LOG_DEBUG("set glove addr : 0x%2x\n",glove_enable_addr);

		retval = synaptics_rmi4_i2c_write(rmi4_data, glove_enable_addr, &ctrl_value, 1);
		if (retval < 0) {
		...
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F01:
				...
				if (status.flash_prog == 1) {
					TS_LOG_ERR("In flash prog mode, status = 0x%02x\n", status.status_code);
