...
#define F54_BUF_LEN 53
#define F54_CHANGE_RATE_OFFSET 38
...
int synaptics_mmi_test(char *buf)
{
	...
	/*I2C communication successfully, go on test*/
	{
		...
		if(V6 == f54->bl_version){
			TS_LOG_INFO("f54->bl_version ==== %d\n", f54->bl_version);
...
static void synaptics_change_report_rate(void)
{
	...
	rc = f54->fn_ptr->write(f54->rmi4_data, f54->control_base_addr + F54_CHANGE_RATE_OFFSET, &command, 1);
	if(rc < 0){
	...
	if((F54_DATA120_RATE == report_rate120) && (F54_DATA60_RATE == report_rate60)) {
		TS_LOG_INFO("change rate success\n");
		strncat(buf_f54test_result, "-4P", MAX_STR_LEN);
	} else {
