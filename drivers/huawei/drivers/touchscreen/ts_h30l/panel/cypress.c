...
#define CY_CHIP_INFO "CYPRESS_TMA463"
...
struct ts_data* g_ts_data_cypress= NULL;
struct touch_settings cypress_sett_param_regs_map[] = {
	[0]={
			.data = (uint8_t *)&cyttsp4_param_regs[0],
			.size = ARRAY_SIZE(cyttsp4_param_regs),
			.tag = 0,
			.file_name = "cyttsp4_fw_h30l.bin",
			.id = OFLIM,
			.module_name = "OFLIM",
		},
	[1]={
...
struct touch_settings *cypress_sett_param_regs = &cypress_sett_param_regs_map[2];
...
static int cypress_wrong_touch(void);
#else
...
/* MUT RAW Range */
#define CY_MUT_RAW_MIN		-3300
#define CY_MUT_RAW_MAX		-1000
/* SELF RAW Range */
#define CY_SELF_RAW_MIN		-800
#define CY_SELF_RAW_MAX 	1600
/* MUT IDAC Range */
...
#endif
#ifdef HUAWEI_EASY_WAKEUP
	.chip_wrong_touch=cypress_wrong_touch,
#endif
 };
...
static int _get_cmd_offs(u8 mode)
{
	int cmd_ofs;
	switch (mode) {
		case CY_HST_CAT:
			TS_LOG_DEBUG("_get_cmd_offs:CY_HST_CAT\n");
			cmd_ofs = CY_REG_CAT_CMD;
			break;
		case CY_HST_OPERATE:
			TS_LOG_DEBUG("_get_cmd_offs:CY_HST_OPERATE\n");
			cmd_ofs = core_data.sysinfo.sysinfo_merged_data.cmd_ofs;
			break;
		default:
			TS_LOG_DEBUG("Unsupported mode 0x%x for exec cmd\n", mode);
			return -EACCES;
	}
	return cmd_ofs;
}

static int _cyttsp4_exec_cmd( u8 mode,u8 *cmd_buf, size_t cmd_size)
{
	...
	cmd_ofs = _get_cmd_offs(mode);
	if (cmd_ofs < 0)
		return -EACCES;

	TS_LOG_DEBUG("%s:cyttsp4_exec_cmd.cmd_buf = %x %x \n", __func__, cmd_buf[0],cmd_buf[1]);
	...
	if (rc < 0) {
		TS_LOG_ERR("%s: Error on read r=%d\n", __func__, rc);
		return rc;
	}
	...
	if (cmd_size > 1) {
		rc = cypress_i2c_write(cmd_param_ofs, cmd_param_buf, cmd_param_size);
		if (rc < 0) {
			TS_LOG_ERR("%s: Error on write command parameters r=%d\n",
				__func__, rc);
			return rc;
		}
	}
	...
	/* Write the command */
	rc =cypress_i2c_write(cmd_ofs, cmd_buf, 1);
	if (rc < 0) {
		TS_LOG_ERR("%s: Error on write command r=%d\n",
				__func__, rc);
		return rc;
	}
	TS_LOG_DEBUG("_cyttsp4_exec_cmd:Write command complete\n ");
	return 0;
}

static int _cyttsp4_put_device_into_sleep(int mode )
{
	u8 command_buf[2];
	u8 device_ctrl;
	int rc;
	TS_LOG_DEBUG("_cyttsp4_put_device_into_sleep\n");
	command_buf[0] = 6;
	command_buf[1] = g_ts_data_cypress->easy_wakeup_info.easy_wakeup_gesture;
	rc = _cyttsp4_exec_cmd(CY_HST_OPERATE,command_buf,2);
	if (rc){
		TS_LOG_ERR(" Error executing command r=%d\n", rc);
	}

	if(g_ts_data_cypress->easy_wakeup_info.easy_wakeup_flag == 1){
			g_ts_data_cypress->easy_wakeup_info.easy_wakeup_flag= 0;
	}
...
#ifdef HUAWEI_EASY_WAKEUP
	if(g_ts_data_cypress->easy_wakeup_info.easy_wakeup_gesture != 0 ){
		rc = _cyttsp4_put_device_into_sleep(mode);
		if (rc) {
			TS_LOG_ERR("Fail easy wakeup write mode r=%d\n", rc);
			goto out;
		}
		TS_LOG_INFO("write EASY WAKEUP SLEEP succeeded\n");
	}else{
		mode |= CY_HST_SLEEP;
...
static int cypress_chip_get_info(struct ts_chip_info_param *info)
{
	...
	memcpy(info->mod_vendor, cypress_sett_param_regs->module_name, strlen(cypress_sett_param_regs->module_name));
...
static int cypress_sysinfo_pcfg(void)
{
	...
	core_data.sysinfo.pcfg.panel_info0 = pcfg->panel_info0;
	for (count =0;cypress_sett_param_regs_map[count].module_name  !=NULL;count++)
	{
		if (core_data.sysinfo.pcfg.panel_info0 == cypress_sett_param_regs_map[count].id)
		{
			cypress_sett_param_regs = &cypress_sett_param_regs_map[count];
			break;
		}
	}
	TS_LOG_INFO("module id is %s\n", cypress_sett_param_regs->module_name);
...
#ifdef HUAWEI_EASY_WAKEUP
	struct ts_easy_wakeup_info *info = &(g_ts_data_cypress->easy_wakeup_info);
	info->easy_wakeup_gesture = 0x00;
	info->easy_wakeup_support = 0x01;
...
static bool cypress_need_fw_update(char *file_name)
{
	...
	rc = request_firmware(&fw_entry, cypress_sett_param_regs->file_name, dev);
	if (rc < 0) {
...
static struct input_dev *hisi_easy_input_dev = NULL;

static int huawei_hisi_easywakeup_register_inputdev()
{
	int ret = 0;
	hisi_easy_input_dev = input_allocate_device();
	if(hisi_easy_input_dev == NULL){
		TS_LOG_ERR("can't alloc input device\n");
		return -EINVAL;
	}
	hisi_easy_input_dev->name = "huawei_easywakeup";
	set_bit(EV_KEY,hisi_easy_input_dev->evbit);
	__set_bit(59, hisi_easy_input_dev->keybit);
	__set_bit(60, hisi_easy_input_dev->keybit);
	__set_bit(61, hisi_easy_input_dev->keybit);
	__set_bit(62, hisi_easy_input_dev->keybit);
	__set_bit(63, hisi_easy_input_dev->keybit);
	__set_bit(116, hisi_easy_input_dev->keybit);
	ret = input_register_device(hisi_easy_input_dev);
	if(ret){
		TS_LOG_ERR("can't alloc input device\n");
		return -EINVAL;
	}	
...
static int cypress_init_chip(struct  ts_data *g_ts_data )
{
	...
	g_ts_data_cypress = g_ts_data;
	huawei_hisi_easywakeup_register_inputdev();
...
static int cypress_wrong_touch()
{
	int rc=NO_ERR;
	rc=cypress_suspend();
	if(rc<0)
	{
		TS_LOG_ERR("Failed to re-suspend, rc = %d\n", rc);
		return rc;
	}
	TS_LOG_INFO("done\n");
	return rc;
...
static void huawei_hisi_easywakeup_report_key(int reportkey){
	unsigned long ids = 0;
	int key = 0;
	TS_LOG_INFO("huawei_hisi_easywakeup_report_key = %d\n",reportkey);
	switch (reportkey){
		case 1:
			key = 59;
			break;
	...
	input_report_key(hisi_easy_input_dev, key, 1);
	input_sync(hisi_easy_input_dev);
	input_report_key(hisi_easy_input_dev, key, 0);
	input_sync(hisi_easy_input_dev);
}
...
#ifdef HUAWEI_EASY_WAKEUP
	TS_LOG_DEBUG("mode[0-3] = 0x%x, 0x%x, 0x%x ,0x%x \n",mode[0], mode[1], mode[2],mode[3]);
#else
	TS_LOG_DEBUG("mode[0-2] = 0x%x, 0x%x, 0x%x\n",mode[0], mode[1], mode[2]);
#endif
...
#ifdef HUAWEI_EASY_WAKEUP
	if ((mode[2] & CY_CMD_MASK) == 6 && mode[2] & CY_CMD_COMPLETE) {
		TS_LOG_INFO("easy_flag = %d \n",g_ts_data_cypress->easy_wakeup_info.easy_wakeup_flag);
		if(g_ts_data_cypress->easy_wakeup_info.easy_wakeup_flag == 0){
			huawei_hisi_easywakeup_report_key(mode[3]&0xffff);	
			g_ts_data_cypress->easy_wakeup_info.easy_wakeup_flag = 1;
...
static int cypress_check_raw_data(void)
{
	...
	enum check_data_type type;
   	for(i = 0; i < 2; ++i){
		if(0 == i){
			type = CY_CHK_MUT_RAW;
			core_data.rawdate.heatmap.numElement = 312;
			core_data.rawdate.heatmap.dataType = CY_MUT_RAW;
		}else{
			type = CY_CHK_SELF_RAW;
			core_data.rawdate.heatmap.numElement = 13;
...
static int cypress_check_short_data ()
{
	int rc =0;
	u8 cmd_buf[2];
	u8 return_buf[3];
	...
	rc = cypress_exec_cmd(CY_HST_CAT, cmd_buf, 2, return_buf, 3);
	if (rc < 0){
		TS_LOG_ERR("exec short test cmd error.\n");
		return rc;
		}
		
	TS_LOG_INFO("short test:return_buf[0]=%x,return_buf[1]=%x,return_buf[2]=%x\n",\
...
static int cypress_get_rawdata(struct ts_rawdata_info *info, struct ts_cmd_node *out_cmd)
{
	...
	rc3 = cypress_check_short_data();
	if(rc3 < 0)
		TS_LOG_ERR("cypress_check_short_data return %d\n", rc3);
...
cypress_get_rawdata:
	if (TS_ACTION_UNDEF == info->op_action) {
		if (rc1 < 0) {
			...
			memcpy(info->buff, "0P-1P-2F-3P", sizeof("0P-1P-2F-3P"));
		}else {
		...
		if (rc1 < 0) {
			...
			memcpy(info->result, "0P-1P-2F-3P", sizeof("0P-1P-2F-3P"));
		} else {
...
static int cypress_check_status(void)
{
	...
	if (core_data.process_state == CY_STATE_NORMAL){
		rc = cypress_i2c_read(&hst_mode_addr, sizeof(hst_mode_addr), &mode, 1);
		if (rc < 0) {
			TS_LOG_ERR("failed to access device in watchdog timer rc=%d \n", rc);
		} else {
			TS_LOG_DEBUG("mode=0x%02x\n",mode);

			cur_mode=mode & CY_HST_MODE;
		
			cmd_buf[0] = CY_CMD_OP_NULL;
			rc = cypress_exec_cmd(cur_mode,cmd_buf, CY_CMD_OP_NULL_CMD_SZ,\
				NULL, CY_CMD_OP_NULL_RET_SZ);
			if (rc < 0) {
			TS_LOG_ERR("Watchdog NULL cmd failed.\n");
			}
		}
	}else{
		TS_LOG_ERR("not in normal state, state = %d\n", core_data.process_state);
