...
struct ts_data* g_ts_data_synaptics= NULL;
...
struct touch_settings synaptics_sett_param_regs_map[] = {
	[0]={
			.build_id[0] = '0',
			.build_id[1] = '0',
			.build_id[2] = '0',
...
struct touch_settings *synaptics_sett_param_regs = &synaptics_sett_param_regs_map[2];
...
synaptics_rmi4_f54_init(rmi4_data); // for test report function
	...
	msleep(1000);
	rc = synaptics_get_fw_data_boot(file_name,synaptics_sett_param_regs);
	if (rc) {
		...
		struct ts_device_data *chip_data, struct platform_device *ts_dev)
{
	...
	rmi4_data->synaptics_chip_data->is_in_cell = false; //reviewed
...
static void synaptics_put_device_into_easy_wakeup(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		TS_LOG_ERR("Failed to set wakeup mode4\n");
		rmi4_data->sensor_sleep = false;
		return;
	} else {
		rmi4_data->sensor_sleep = true;
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F51:
				...
				TS_LOG_INFO("rmi_fd.intr_src_count =0x%x",rmi_fd.intr_src_count);			
				fhandler = synaptics_rmi4_alloc_fh(&rmi_fd, page_number);
				if (NULL == fhandler) {
					TS_LOG_ERR("Failed to alloc for F%d\n",rmi_fd.fn_number);
					return retval;
...
static int synaptics_input_config(struct input_dev *input_dev)
{
	...
	/* BEGIN PN: , Modified by l00184147, 2014/4/9*/
	...
	/* END PN: , Modified by l00184147, 2014/4/9*/
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	switch(device_data)
	{
		...
		case SPECIFIC_LETTER_DETECTED:
			...
			TS_LOG_DEBUG("%s:###read### f11_2d_data39[8] = 0x%x\n",__func__,f11_2d_data39[8]);
			...
			switch(f11_2d_data39[8])
			{
				...
				case SPECIFIC_LETTER_w:
					...
					break;
				default:
					TS_LOG_INFO("@@@unknow letter!f11_2d_data39[8] = %d@@@\n",f11_2d_data39[8]);
					return 0;
			}
			break;
		default:
	...
	{			
		...
		if(1==off_motion_on)
		{
			...
			TS_LOG_INFO("Gesture is  reported \n");
			if (retval < 0)
			{
				mutex_unlock(&wrong_touch_lock);
				return 0;
