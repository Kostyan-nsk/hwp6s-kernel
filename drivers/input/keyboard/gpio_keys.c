...
static void gpio_keys_report_event(struct gpio_button_data *bdata)
{
	...
	if (!state && (KEY_VOLUMEDOWN == button->code)) {
		wake_unlock(&volume_down_key_lock);
	} else if (!state && (KEY_VOLUMEUP == button->code)){
...
static void gpio_keys_work_func(struct work_struct *work)
{
	...
	if (!(bdata->timer_debounce)) {
		...
		if (!key_event && (KEY_VOLUMEDOWN == button->code)) {
			wake_lock(&volume_down_key_lock);
		} else if (!key_event && (KEY_VOLUMEUP == button->code)) {
			wake_lock(&volume_up_key_lock);
		} else {
			/* not the volume key, we have nothing to do */
		}
	} else {
		/* we have wakelock in the gpio keys timer function */
	}
...
int is_gpio_key_vol_updown_pressed(void)
{
	...
	if ((vol_up_gpio<0) || (vol_down_gpio<0)
		...
		printk(KERN_ERR "[%s]: vol_updown gpio or active_low is invalid!\n", __FUNCTION__);
		return 0;
	}
...
static int __devinit gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		//for vol_updown key combination
		if (KEY_VOLUMEDOWN == button->code){
			vol_down_gpio = button->gpio;
			vol_down_active_low = button->active_low;
		}
		
		if (KEY_VOLUMEUP == button->code){
			vol_up_gpio = button->gpio;
			vol_up_active_low = button->active_low;
		}
...
static int gpio_keys_resume(struct device *dev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if ((KEY_VOLUMEDOWN == button->code) || (KEY_VOLUMEUP == button->code)) {
			continue; //ignore the simulate report event on volume key for special feature
		}
