...
static void ispv1_focus_get_curr_data(mini_focus_frame_stat_s *curr_data)
{
	...
	print_info("current contrast 0x%.3x, lum 0x%.2x, xyz: %.4d, %.4d, %.4d",
...
void misp_gyro_info(void)
{
	...
	/* added for delay 5 frames after CAF triggered. */
	static int frame_count = 0;
	static bool global, local;
	static bool last_global, last_local;
	static bool flag = false;

	last_global = global_motion;
	last_local = local_motion;
	...
	// added for delay 5 frames for last CAF.
	if (last_global == false && last_local == false && (global == true || local == true))
	{
		flag = true;
		frame_count = 0;
	}
