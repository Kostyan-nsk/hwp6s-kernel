...
struct mipi_dsi_phy_register phy_ctrl[5] = {{102, 34, 124, 47, 8, 11, 8, 0x30, 3, 19, 132, 10},
...
DECLARE_WAIT_QUEUE_HEAD(overlay_idle_wait_queue);
...
static enum hrtimer_restart k3fb_cmd_esd(struct hrtimer *timer)
{
	...
	hrtimer_start(&k3fd->esd_hrtimer, ktime_set(k3fd->esd_timer_interval, NSEC_PER_SEC), HRTIMER_MODE_REL);
...
void k3_fb_set_backlight(struct k3_fb_data_type *k3fd, u32 bkl_lvl)
{
	...
	if (k3fd->panel_info.type == PANEL_MIPI_CMD) {
		if (k3fd->cmd_mode_refresh || k3fd->checkesd_bl_cannot_set ||(!k3fd->cmd_bl_can_set)) {
...
STATIC int k3_fb_probe(struct platform_device *pdev)
{
	...
	k3fd->esd_timer_interval = 0;
	ret= get_hw_config_int("lcd/esd_check_type", &k3fd->esd_timer_interval, NULL); 
	return 0;
}
