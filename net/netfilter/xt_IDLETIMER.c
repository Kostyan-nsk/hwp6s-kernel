...
{
	int err;
    pr_crit("[MTU]%s:enter\n", __func__);//liubinjun 20150610
	idletimer_tg_class = class_create(THIS_MODULE, "xt_idletimer");
	err = PTR_ERR(idletimer_tg_class);
	if (IS_ERR(idletimer_tg_class)) {
		pr_debug("couldn't register device class\n");
        pr_crit("[MTU]%s:couldn't register device class\n", __func__);//liubinjun 20150610
		goto out;
	}
	...
	if (IS_ERR(idletimer_tg_device)) {
        ...
        pr_crit("[MTU]%s:couldn't register system device\n", __func__);//liubinjun 20150610
		goto out_class;
	}
	...
	if (err < 0) {
        pr_crit("[MTU]%s:couldn't register xt target,err=%d\n", __func__,err);//liubinjun 20150610
